...
const CartDetails = ({ onClose }) => {
  ...
  const handleDeleteCart = (newCartItem) => {
    const filterCart = cartMovie.filter((item) => item.id !== newCartItem.id);
    setCartMovie(filterCart);
  };
  return (
    <div>
      <div className="fixed top-0 left-0 w-screen h-screen z-50 bg-black/60 backdrop-blur-sm">
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[790px] p-4 max-h-[90vh] overflow-auto">
          <div className="bg-white shadow-md dark:bg-[#12141D] rounded-2xl overflow-hidden p-5 md:p-9">
            ...
            {
              cartMovie.length === 0 ? <p className="text-2xl text-red-500">The Cart is Empty.</p> :
            (cartMovie.map((item) => (
              <div
                key={item.id}
                className="space-y-8 lg:space-y-12 max-h-[450px] overflow-auto mb-10 lg:mb-14 border border-[#D42967] px-2 py-2"
              >
            ...
            <div className="flex items-center justify-end gap-2">
              <a
                className="rounded-md p-2 md:px-4 inline-flex items-center space-x-2 bg-[#00d191] text-[#171923] text-sm"
                href="#"
              >
